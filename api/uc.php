...
if(!defined('IN_UC')) {
	...
	require_once DISCUZ_ROOT.'../data/uc_conf/'.$domain['0'].'/config.inc.php';
...
class uc_note {
	...
	function uc_note() {
		...
		$this->dbconfig = DISCUZ_ROOT.'../data/uc_conf/'.$domain['0'].'/config.inc.php';
	...
	function synlogin($get, $post) {
		...
		_setcookie(THINKCMF_AUTH, _authcode($uid."\t".$username, 'ENCODE'),7*24*3600);
	...
	function updateapps($get, $post) {
		...
		if(is_writeable(DISCUZ_ROOT.'../data/uc_conf/'.$domain['0'].'/config.inc.php')){

			$configfile = trim(file_get_contents(DISCUZ_ROOT.'../data/uc_conf/'.$domain['0'].'/config.inc.php'));

			$configfile = substr($configfile, -2) == '?>' ? substr($configfile, 0, -2) : $configfile;

			$configfile = preg_replace("/define\('UC_API',\s*'.*?'\);/i", "define('UC_API', '$UC_API');", $configfile);

			if($fp = @fopen(DISCUZ_ROOT.'../data/uc_conf/'.$domain['0'].'/config.inc.php', 'w')) {
