;(function () {
    ...
    var yearmonthInput = $("input.js-yearmonth");
    if (yearmonthInput.length) {
        Wind.use('datePicker', function () {
            yearmonthInput.datePicker({
                startView: 'decade',
                minView: 'decade',
                format: 'yyyymm',
                autoclose: true
    ...
    var date_otherInput = $("input.js-date_other");
    if (date_otherInput.length) {
    	Wind.use('datePicker', function () {
    		date_otherInput.datePicker(
    		{ format 	: 'yyyymmdd'}
    		);
    	});
    }
...
function upload_one(dialog_title, input_selector, filetype, extra_params, app) {
    open_upload_dialog(dialog_title, function (dialog, files) {
        $(input_selector + '-url').val(files[0].preview_url);
