var exam={
	...
	init:function(){
		...
		//批量添加事件初始化
		this.topicACtrlFn('.ui-questions-content-list','.ui-add-item-btn','.ui-batch-item-btn','.ui-add-answer-btn','.ui-del-item-btn');
		//
	...
	//单题添加，批量添加
	topicACtrlFn:function(parentObj,addObj,batchAddObj,addAnswerObj,delObj){
		//添加选项栏
		...
		//删除选项栏
		$(document).on('click',parentObj+' '+delObj,function (e) {
            var count = $(this).closest('li.ui-module').find('.cq-unset-list li').length;
            if(count == 1){
                layer.msg('必须保留一个选项');
                return false;
			}
            $(this).closest('li.ui-module').find('.cq-unset-list li:eq('+(count - 1)+')').remove();
