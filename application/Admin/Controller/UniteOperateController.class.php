...
class UniteOperateController extends AdminbaseController{
    ...
    public function index(){
        ...
        $count = M('unite_channel')->where($where)->count();
        $page = $this->page($count,20);
        $this->page = $page->show('Admin');
        $this->data = M('unite_channel')->where($where)->limit($page->firstRow,$page->listRows)->order('id desc')->select();
    ...
    public function addChannel(){
        if(IS_POST){
            ...
            if(M('unite_channel')->where(array('cid'=>$data['cid']))->find()){
                $this->error('该渠道已经创建过数据');
            }
            if($data['tag'] == ''){
                $this->error('拼音简写不能为空');
            }
            if(M('unite_channel')->where(array('tag'=>$data['tag']))->find()){
                $this->error('拼音简写不能重复');
            }
    ...
    public function editChannel(){
        if(IS_POST){
            ...
            if(M('unite_channel')->where(array('cid'=>$data['cid'],'id'=>array('neq',$data['id'])))->find()){
                $this->error('该渠道已经创建过数据');
            }
            if($data['tag'] == ''){
                $this->error('拼音简写不能为空');
            }
            if(M('unite_channel')->where(array('tag'=>$data['tag'],'id'=>array('neq',$data['id'])))->find()){
                $this->error('拼音简写不能重复');
            }
            if($data['packname'] == '' || strrpos($data['packname'],'#tag#') === false){
                $this->error('包名格式请严格按照示例填写');
            ...
            $id = I('id');
            $data = M('unite_channel')->where(array('id'=>$id))->find();
    ...
    public function delChannel(){
        ...
        if(M('unite_channel')->where(array('id'=>array('in',$id)))->delete()){
            $this->success('操作成功');
        }else{
            $this->error('操作失败');
        }
    ...
    public function subIndex(){
        ...
        $count = M('subpackage_unite')->where($where)->count();
        $page = $this->page($count,20);
        $data = M('subpackage_unite')->where($where)->limit($page->firstRow,$page->listRows)->order('id desc')->select();
        foreach($data as $k=>&$v){
    ...
    public function subAdd(){
        if(IS_POST){
            ...
            if(M('subpackage_unite')->where(array('appid'=>$data['appid'],'masterid'=>$data['masterid']))->find()){
...
//                    dump($path);
            ...
            $this->cdata = M('unite_channel')->select();
            $this->channel = M('uchannelmaster',null,C('RH_DB_CONFIG'))->getfield('masterID,masterName',true);
            $this->gameList = get_game_list('',2,'all','all','all',1);
            $this->display();
        }
    }
    ...
    public function subEdit(){
        if(IS_POST){
            ...
            if($res['state'] == 1){
                ...
                if(M('subpackage_unite')->save($edit)){
                    $this->success('操作成功');
                }else{
                    $this->error('操作失败');
                }
            ...
            $id = I('id');
            $data = M('subpackage_unite')->where(array('id'=>$id))->find();
    ...
    public function subDel(){
        ...
        if(M('subpackage_unite')->where(array('id'=>array('in',$id)))->delete()){
            $this->success('操作成功');
        }else{
            $this->error('操作失败');
        }
    ...
    public function doPackage(){
        ...
        $url = $pakcOrretry == 1 ? C('RH_ANDROID_URL') : C('RH_RETRY_ONE');
        $res = curl_post($url,$sub);
        $res = json_decode($res,true);
        if(is_array($res)){
            if($res['state'] == 1){
                M('subpackage_unite')->where(array('id'=>$id))->setField(array('status'=>2,'sub_time'=>time()));
                $this->success('正在出包中,请稍后刷新进度');
            }elseif($res['state'] == 6){
                M('subpackage_unite')->where(array('id'=>$id))->setField(array('status'=>1,'downurl'=>$res['url'],'sub_time'=>time()));
                $this->success('分包完成');
            }else{
    ...
    public function checkProgress(){
        ...
        $res = curl_post(C('RH_CHECK_ONE'),$sub);

        $res = json_decode($res,true);
        if(is_array($res)){
            if($res['state'] == 6){
                M('subpackage_unite')->where(array('id'=>$id))->setField(array('status'=>1,'downurl'=>$res['url']));
                $this->success('分包完成');
            }else{
                if($res['state'] == 7){
                    M('subpackage_unite')->where(array('id'=>$id))->setField(array('status'=>3));
                }
                $this->error(getPackError($res['state']));
            }
        }else{
    ...
    public function getPackName(){
        ...
        if(!$appid || !$cid){
            ...
            if($info['tag'] == ''){
                ...
                $pack = M('unite_channel')->field('packname,tag,again_sub')->where(array('cid'=>$cid))->find();
    ...
    public function getParams(){
        $cid = I('cid');
        $data = M('unite_channel')->where(array('cid'=>$cid))->getField('parameter');
        if($data){
    ...
    public function uploadfile($file,$root,$name){
        ...
        $upload->driverConfig = array(
            ...
            'timeout'  => '900',
            'username' => 'ftplianyun',
            'password' => 'Ftp@3232HDkd!@#efb|fsd7VM#kj&'
        );
