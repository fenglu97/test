...
class SettingController extends AdminbaseController{
	...
	public function site_post(){
		if (IS_POST) {
			...
			if(is_array($options['change_game']))
			{
				foreach($options['change_game'] as $k=>$v)
				{
					$options['change_game'][$k]['content'] = trim($v['content']);
				}
			}

			if(is_array($options['app_promise']))
			{
				foreach($options['app_promise'] as $k=>$v)
				{
					$options['app_promise'][$k]['content'] = trim($v['content']);
				}
			}

			if(is_array($options['buyer_notes']))
			{
				 $options['buyer_notes'] = array_map("trim",$options['buyer_notes']);
			}

			if(is_array($options['seller_notes']))
			{
				$options['seller_notes'] = array_map("trim",$options['seller_notes']);
			}

			if(is_array($options['business_notice']))
			{
				$options['business_notice'] = array_map("trim",$options['business_notice']);
			}

			if(is_array($options['consult']['protocol']))
			{
				$options['consult']['protocol'] = array_map("trim",$options['consult']['protocol']);
			}

			if(is_array($options['consult']['notice']))
			{
				$options['consult']['notice'] = array_map("trim",$options['consult']['notice']);
			}
	...
	public function clearcache(){
		sp_clear_cache();
		//updateCache(C('UPDATE_CACHE_URL'),'clearCache','all','6e179c39af339f13183cbaa1040e3e98');
		$this->display();
	}
	...
	public function agreement(){
	    if(IS_POST){
            $data = I('agreement');
            if(M('options')->where(array('option_name'=>'agreement'))->setField('option_value',$data) !== false){
                $this->success();
            }else{
                $this->error('操作失败');
            }
        }else{
	        $data = M('options')->where(array('option_name'=>'agreement'))->getField('option_value');
	...
	public function hide()
	{
		if(IS_POST){
            $data = I('hide');
            if(M('options')->where(array('option_name'=>'hide'))->setField('option_value',$data) !== false){
                $this->success();
            }else{
                $this->error('操作失败');
            }
        }else{
	        $data = M('options')->where(array('option_name'=>'hide'))->getField('option_value');
	        $this->data = html_entity_decode($data);
	        $this->display();
        }
	}
