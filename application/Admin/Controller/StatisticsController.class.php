...
class StatisticsController extends AdminbaseController{
	...
	public function pay_statistics()
	{
		...
		if($action == 0)
		{
			...
			$this->assign('game_list',get_game_list(I('appid'),1, 1,'all','all','all','',1));
		...
		{
			//导出模式
			$xlsTitle = iconv('utf-8', 'gb2312', '充值统计');//文件名称
			$fileName = date('_YmdHis').'订单统计';//or $xlsTitle 文件名称可根据自己情况设定

			$expCellName = array('日期','游戏名称','渠道名称','活跃用户','充值次数','充值人数','总充值金额',
                '新增SDK用户','新增小号','新增小号(排重)','新增小号充值次数',
                '新增小号充值人数','新增小号充值金额','活跃Arpu','新增用户Arpu','付费率');
	...
	public function today_pay()
	{
		...
		else
		{
			...
			{
				$map['appid'] = array('in',implode(',',$this->appids));
				$map1['appid'] = array('in',implode(',',$this->appids));
		...
		$map1['create_time'] = array(array('egt',$today_day),array('lt',$today_day+3600*24));
...
//		->select();
		...
		->where(array_merge($map1,array('status'=>1)))
		->group('appid')
		->buildSql();
...
//		->select();
		...
		->where(array_merge($iMap,array('status'=>1)))
		->group('cid')
		->select();
	...
	{
		dump(upload_token('zhoulin','aa123456'));
	}
