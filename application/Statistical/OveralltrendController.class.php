 ...
 * @date 2017-05-23
 */
namespace Statistical\Controller;
...
class OveralltrendController extends AdminbaseController{
	...
	public function index()
	{
		...
		$box_install_info_obj = M('box_install_info','bt_','185DB');
		$box_boot_info_obj = M('box_boot_info','bt_','185DB');
		...
		->where(array('create_time'=>array(array('egt',$today-3600*24*29),array('lt',$today+3600*24))))
		...
		foreach($datearr as $v)
		{
			...
			$result[] = array($v,$xinzen,$huoyue,$boot);
		}
		...
		$this->assign('before_30_date',date('Y-m-d',strtotime('-29 days')));
